...
export default function WorkerPurchasesPage() {
    ...
    const loadData = async () => {
        ...
        const todaysPurchases = data.filter(p => p.date === today);

        // RLS ensures the worker only sees their own purchases, and we filter by today
        setPurchases(todaysPurchases);
        setLoading(false);
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-fiori-header">Mis Compras</h1>
                    <p className="text-fiori-text-light">Registra las compras de mercadería</p>
                </div>
                <button
                        ...
                        {purchases.length === 0 ? (
                            <tr>
                                <td colSpan={7} className="px-6 py-10 text-center text-gray-500">
                                    No has registrado compras el día de hoy.
                                </td>
                            </tr>
                        ) : (
